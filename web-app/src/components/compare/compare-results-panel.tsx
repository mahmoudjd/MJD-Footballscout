...
function metricCardValueClass(highlighted: boolean) {
  return cn(
    ...
    highlighted ? "border-cyan-200 bg-cyan-50 text-cyan-800" : "border-slate-200 text-slate-700",
  )
}
...
function metricTableCellClass(highlighted: boolean) {
  return cn(
    "px-3 py-2",
    highlighted ? "bg-cyan-50 font-semibold text-cyan-800" : "text-slate-700",
  )
}
...
function ScoreboardCards({
  ...
  return (
    <div className="space-y-3 md:hidden">
      {players.map((player) => (
        <article key={player._id} className="rounded-lg border border-slate-200 p-3">
      ...
      <table className="min-w-full divide-y divide-slate-200 text-xs lg:text-sm">
        <thead className="bg-slate-50">
          <tr>
            ...
            <th className="px-3 py-2 text-left font-semibold text-slate-700">Age</th>
        ...
        <tbody className="divide-y divide-slate-100">
          {players.map((player) => (
            <tr key={player._id} className="align-top">
              <td className="px-3 py-2 text-slate-600">{rankOrderById.get(player._id) || "-"}</td>
              ...
              <td className="px-3 py-2 text-slate-700">{scoreById.get(player._id) ?? 0}</td>
